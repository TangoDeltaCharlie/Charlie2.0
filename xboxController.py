...
"""
NOTES - pygame events and values as mapped in Computer
JOYAXISMOTION
...
//THIS IS NOT USED ON MAC OS X ---------- MAYBE USED ON WINDOWS????
JOYHATMOTION
event.value
[0] - horizontal
[1] - vertival
...
class XboxController(threading.Thread):
    ...
    """
    BUTTONCONTROLMAP = {PyGameButtons.DPADUP: XboxControls.DPADUP,
    ...
    def _start(self):
        ...
        while (self.running):
            ...
            if settings.quitProgram:
                print("Quitting Xbox Controller Thread")
                self.stop()
    ...
    def loadCallbacks(self):
        self.setupControlCallback(self.XboxControls.LTHUMBX, self.leftThumbX)
        self.setupControlCallback(self.XboxControls.LTHUMBY, self.leftThumbY)
        self.setupControlCallback(self.XboxControls.RTHUMBX, self.rightThumbX)
        self.setupControlCallback(self.XboxControls.RTHUMBY, self.rightThumbY)
        self.setupControlCallback(self.XboxControls.LTRIGGER, self.lTriggerPressed)
        self.setupControlCallback(self.XboxControls.RTRIGGER, self.rTriggerPressed)

        self.setupControlCallback(self.XboxControls.A, self.aPressed)
        self.setupControlCallback(self.XboxControls.B, self.bPressed)
        self.setupControlCallback(self.XboxControls.X, self.xPressed)
        self.setupControlCallback(self.XboxControls.Y, self.yPressed)
        self.setupControlCallback(self.XboxControls.DPADUP, self.dpadUpPressed)
        self.setupControlCallback(self.XboxControls.DPADDOWN, self.dpadDownPressed)
        self.setupControlCallback(self.XboxControls.DPADLEFT, self.dpadLeftPressed)
        self.setupControlCallback(self.XboxControls.DPADRIGHT, self.dpadRightPressed)
        self.setupControlCallback(self.XboxControls.START, self.startPressed)
        self.setupControlCallback(self.XboxControls.BACK, self.backPressed)
        self.setupControlCallback(self.XboxControls.LEFTTHUMB, self.leftThumbPressed)
        self.setupControlCallback(self.XboxControls.RIGHTTHUMB, self.rightThumbPressed)
        self.setupControlCallback(self.XboxControls.LB, self.lbPressed)
        self.setupControlCallback(self.XboxControls.RB, self.rbPressed)
        self.setupControlCallback(self.XboxControls.XBOX, self.xboxPressed)

    def controlCallBack(self, xboxControlId, value):
    ...
    def bPressed(self, value):
        if value == 0:

            if settings.pause_menu_is_up:
                settings.pause_menu_is_up = False

            elif settings.low_power_mode_screen_menu_is_up:
                settings.low_power_mode_screen_menu_is_up = False

            elif settings.high_temp_screen_menu_is_up:
                settings.high_temp_screen_menu_is_up = False

            elif settings.buoyancy_calibration_menu_is_up:
                settings.buoyancy_calibration_menu_is_up = False
            else:
                settings.xbox_b_pressed = True
    ...
    def dpadDownPressed(self, value):
        if settings.program_running:
            if value == 0:
                if not settings.pause_menu_is_up \
                        and not settings.high_temp_screen_menu_is_up \
                        and not settings.low_power_mode_screen_menu_is_up \
                        and not settings.buoyancy_calibration_menu_is_up:
                    settings.high_temp_screen_menu_is_up = True
                print("dpadDown Pressed")

    def dpadLeftPressed(self, value):
        if settings.program_running:
            if value == 0:
                if not settings.pause_menu_is_up \
                        and not settings.high_temp_screen_menu_is_up \
                ...
                print("dpadLeft Pressed")

    def dpadRightPressed(self, value):
        if settings.program_running:
            if value == 0:
                if not settings.pause_menu_is_up \
                        and not settings.high_temp_screen_menu_is_up \
                        and not settings.low_power_mode_screen_menu_is_up \
                        and not settings.buoyancy_calibration_menu_is_up:
                    settings.buoyancy_calibration_menu_is_up = True
                print("dpadRight Pressed")

    def startPressed(self, value):
        if settings.program_running:
            if value == 0:
                if not settings.pause_menu_is_up \
                        and not settings.high_temp_screen_menu_is_up \
